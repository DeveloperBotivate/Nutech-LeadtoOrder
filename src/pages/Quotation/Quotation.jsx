...
const image1 = botivateLogo
const imageform = botivateLogo
...
function Quotation() {
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      // Get the PDF URL
      const permanentPdfUrl = pdfUrlResult || "http://mock-pdf-url.com";

      // 2. Submit "Send" entry
      const sendResult = await mockApi.saveQuotation({
        ...quotationData,
      ...
      if (!sendResult.success) {
        throw new Error("Failed to save to Send sheet or send email");
      }
  ...
  const handleSaveQuotation = async () => {
    ...
    try {
      ...
      const savedPdfFileName = `Quotation_${finalQuotationNo}.pdf`

      // 1. Upload PDF
      const pdfUrlResult = await mockApi.uploadFile({
        name: savedPdfFileName,
        type: "application/pdf",
        base64Data: base64Data
      }, "pdf");
      ...
      // 2. Save Quotation and Items
      const sheetResult = await mockApi.saveQuotation({
        ...quotationData,
      ...
      await Promise.all([]) // Mock items already handled via saveQuotation
          ...
          <div className="flex">
            <button
              className={`px-4 py-2 font-medium ${activeTab === "edit"
                ? "bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-tl-lg"
                : "text-gray-600"
                }`}
            ...
            <button
              className={`px-4 py-2 font-medium ${activeTab === "preview" ? "bg-gradient-to-r from-sky-500 to-blue-600 text-white" : "text-gray-600"
                }`}
      ...
      {activeTab === "edit" && (
        <div className="flex justify-between mt-4">
          ...
          <div className="space-x-2">
            ...
            <button
              className="bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-md flex items-center inline-flex"
