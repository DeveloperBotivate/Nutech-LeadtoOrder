...
import { getCreditLimits, saveCreditLimits } from '../../utils/storageManager';
import { generateId } from '../../utils/helpers';
import DataTable from '../../components/DataTable';
import ModalAlert from '../../components/ModalAlert';
import ModalForm from '../../components/ModalForm';

export default function CreditLimit({ searchQuery, triggerAdd }) {
  ...
  const headers = ['Actions', 'Timestamp', 'CL-NO', 'Credit Limit'];

  useEffect(() => {
    setData(getCreditLimits());
  }, []);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      return (
        ...
        item.clNo?.toLowerCase().includes(q)
      );
    });
  }, [data, searchQuery]);
  ...
  const handleDelete = (id) => {
    showAlert('confirm', 'Delete Record?', 'Are you sure you want to delete this record?', () => {
      const updated = data.filter(i => i.id !== id);
      saveCreditLimits(updated);
      setData(updated);
      showAlert('success', 'Deleted!', 'The record has been removed successfully.');
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (validEntries.length === 0) {
      showAlert('error', 'Error', 'Please provide at least one entry.');
      return;
    }
    ...
    if (editingId) {
      ...
      saveCreditLimits(updated);
      setData(updated);
      showAlert('success', 'Updated!', 'Record has been updated.');
    } else {
      const newItems = validEntries.map((entry, index) => ({
        id: generateId() + index,
        name: entry.name,
        timestamp: new Date().toISOString(),
        clNo: `CL-${String(data.length + index + 1).padStart(3, '0')}`
      }));

      const updatedData = [...data, ...newItems];
      saveCreditLimits(updatedData);
      setData(updatedData);
      showAlert('success', 'Success!', `${newItems.length} record(s) added successfully.`);
    }
    setShowModal(false);
  };
  ...
  const renderRow = (item) => (
    <tr key={item.id} className="hover:bg-gray-50 transition-colors text-center text-sm">
      ...
      <td className="px-4 py-3 text-gray-900 font-bold whitespace-nowrap">{item.clNo}</td>
  ...
  const renderCard = (item) => (
    <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3 relative">
      <div className="flex justify-between items-center border-b border-gray-100 pb-2">
        <div><span className="text-[10px] font-medium text-indigo-600 uppercase tracking-widest">{item.clNo}</span><h3 className="text-sm font-medium text-gray-700 mt-1 uppercase">{item.name}</h3></div>
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <ModalForm
        ...
        title={editingId ? 'Edit Credit Limit' : 'Register Credit Limit'}
        ...
        <div className="space-y-2">
            ...
            {entries.map((entry, index) => (
              <div key={index} className="flex gap-2 items-center group">
                <div className="flex-1 space-y-1">
                  {index === 0 && <label className="block text-[10px] font-medium text-gray-700 uppercase tracking-tight">Credit Limit *</label>}
                  <input required type="text" value={entry.name} onChange={(e) => handleEntryChange(index, e.target.value)} className="w-full border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-500 text-[11px] md:text-[13px] h-[30px] md:h-[34px] uppercase" placeholder="Enter amount" />
                </div>
                {!editingId && entries.length > 1 && (
                  <button type="button" onClick={() => handleRemoveRow(index)} className="mt-auto mb-1 text-red-400 hover:text-red-600 transition-colors p-1">
                    <Minus size={16}/>
                  </button>
                )}
              </div>
          ...
          {!editingId && (
            <div className="pt-2 border-t border-gray-50 flex justify-end">
              <button type="button" onClick={handleAddRow} className="flex items-center gap-1.5 text-[10px] font-black bg-indigo-600 text-white px-3 py-1.5 rounded shadow-sm hover:bg-indigo-700 transition-all active:scale-95 uppercase tracking-widest">
                <Plus size={14}/> Add
              </button>
            </div>
          )}
