...
import { getCreditDays, saveCreditDays } from '../../utils/storageManager';
import { generateId } from '../../utils/helpers';
import DataTable from '../../components/DataTable';
import ModalAlert from '../../components/ModalAlert';
import ModalForm from '../../components/ModalForm';

export default function CreditDays({ searchQuery, triggerAdd }) {
  ...
  const headers = ['Actions', 'Timestamp', 'CD-NO', 'Credit Days'];

  useEffect(() => {
    setData(getCreditDays());
  }, []);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      return (
        ...
        item.cdNo?.toLowerCase().includes(q)
      );
    });
  }, [data, searchQuery]);
  ...
  const handleSubmit = (e) => {
    ...
    if (editingId) {
      ...
      const newItems = validEntries.map((entry, index) => ({
        ...
        cdNo: `CD-${String(data.length + index + 1).padStart(3, '0')}`
      }));

      const updatedData = [...data, ...newItems];
      saveCreditDays(updatedData);
      setData(updatedData);
  ...
  const renderRow = (item) => (
    <tr key={item.id} className="hover:bg-gray-50 transition-colors text-center text-sm">
      ...
      <td className="px-4 py-3 text-gray-900 font-bold whitespace-nowrap">{item.cdNo}</td>
  ...
  const renderCard = (item) => (
    <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3 relative">
      <div className="flex justify-between items-center border-b border-gray-100 pb-2">
        <div><span className="text-[10px] font-medium text-indigo-600 uppercase tracking-widest">{item.cdNo}</span><h3 className="text-sm font-medium text-gray-700 mt-1 uppercase">{item.name}</h3></div>
  ...
  return (
    <div className="flex flex-col flex-1 w-full min-h-0">
      ...
      <ModalForm
        ...
        title={editingId ? 'Edit Credit Days' : 'Register Credit Days'}
            ...
            {entries.map((entry, index) => (
              <div key={index} className="flex gap-2 items-center group">
                <div className="flex-1 space-y-1">
                  {index === 0 && <label className="block text-[10px] font-medium text-gray-700 uppercase tracking-tight">Credit Days *</label>}
                  <input required type="text" value={entry.name} onChange={(e) => handleEntryChange(index, e.target.value)} className="w-full border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-500 text-[11px] md:text-[13px] h-[30px] md:h-[34px] uppercase" placeholder="Enter days" />
                </div>
